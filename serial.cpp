...
bool Serial::open(QString serialName, int baudRate)
{
    ...
    if(m_serialPort->open(QIODevice::ReadWrite))
    {
        ...
        qDebug() << "Open "+serialName+" successd!!";
...
void Serial::sendData(QByteArray &sendData)
{
    ...
    qDebug() << "serialport send" << sendData.size()<<":"<<sendData.toHex();
...
void Serial::readData()
{
    ...
    qDebug() << "serialport read" << m_readBuf.toHex();
...
QByteArray Serial::hexStringToByteArray(QString HexString)
{
    ...
    foreach (QString s, sl)
    {
        if(!s.isEmpty())
        {
            ushort c = static_cast<char>(s.toUInt(&ok, 16) & 0xFF);
            if(ok)
            {
                ret.append(c);
                qDebug()<<c;
            ...
            {
                qDebug()<<"非法的16进制字符："<<s;
                QMessageBox::warning(nullptr, tr("ERROR："), QString("非法的16进制字符: \"%1\"").arg(s));
            }
        }
    }
